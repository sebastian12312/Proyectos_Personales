...
public class MisCompras extends HttpServlet {
    ...
    ComprasUsuario comprasUser= new ComprasUsuario();
    ArrayList<ComprasUsuario> compras = new ArrayList<>();
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession session = request.getSession();
        String codigoUsuario = (String)session.getAttribute("CodigoUsuario");
        if (codigoUsuario != null) {            
            compras = funcionUsuario.ListarCompras(codigoUsuario);          
            session.setAttribute("ListaCompras", compras);
            response.sendRedirect("usuario/MisCompras.jsp");
        }else{
